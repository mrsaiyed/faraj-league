...
export function renderStats() {
  ...
  const tbodyRows = rows.map((r, i) => {
    ...
    return `<tr><td style="padding:0.7rem 1rem;color:#c8c0b0;font-size:0.82rem">${i + 1}</td><td style="padding:0.7rem 1rem"><div style="font-size:0.92rem;color:#f5f0e8;font-weight:600">${escapeHtmlAttr(r.name)}</div><div style="font-size:0.78rem;color:#2fa89a;letter-spacing:0.05em;margin-top:0.1rem">${escapeHtmlAttr(r.team)}${escapeHtmlAttr(teamRec(r.team))}</div></td><td style="padding:0.7rem 1rem">${r.gp}</td>${defCells}<td style="padding:0.7rem 1rem;color:#2fa89a">${ppg > 0 ? ppg.toFixed(1) : '—'}</td></tr>`;
  }).join('');
